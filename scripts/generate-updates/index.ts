...
const main = async () => {
  ...
  // INFO: generate-updates-on
  {
    ...
    for (const [key, value] of Object.entries(UpdateType)) {
      clauses.push(`on<T = {}>(events: UpdateType.${key} | '${value}', handlers: MaybeArray<Middleware<Contexts.${getContext(key, value)} & T>>): this`)
    }

    clauses.push('on<T = {}>(events: MaybeArray<string>, handlers: MaybeArray<Middleware<Contexts.Context & T>>): this')
