...
Контекст, наследуемый от `PreCheckoutQuery` и [`Context`](context.md).

**Вызывается, когда боту приходит `PreCheckoutQuery`.**

```ts
import { PreCheckoutQueryContext } from 'puregram';
```
...
| `update`   | `TelegramPreCheckoutQuery` | [Объект события][TelegramPreCheckoutQuery] |

[TelegramPreCheckoutQuery]: https://core.telegram.org/bots/api#precheckoutquery
