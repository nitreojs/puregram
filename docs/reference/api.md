...
puregram exposes the bot api at three levels of abstraction (see the three-layer api concept):
...
- **`tg.send(args)`** — curated layer. a hand-selected set of high-level helpers that wrap the response into a `Message` (or similar) for chainability. covers the most common send operations.
