...
**Вызывается, когда группа перешла из группы.**

```ts
import { MigrateFromChatIdContext } from 'puregram';
```
...
```ts
const context: MigrateFromChatIdContext = new MigrateFromChatIdContext(telegram, update);
```
