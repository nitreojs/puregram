...
Контекст, наследуемый от `PollAnswer` и [`Context`](context.md).

**Вызывается, когда пользователь ответил на НЕ анонимный опрос, созданный ботом.**

```ts
import { PollAnswerContext } from 'puregram';
```
...
```ts
const context: PollAnswerContext = new PollAnswerContext(telegram, update);
```

|  Параметр  |          Тип         |                Описание              |
| :--------: | :------------------: | :----------------------------------: |
| `telegram` | `Telegram`           | Инстанция `Telegram`                 |
| `update`   | `TelegramPollAnswer` | [Объект события][TelegramPollAnswer] |

[TelegramPollAnswer]: https://core.telegram.org/bots/api#pollanswer
