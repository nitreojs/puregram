...
Контекст, наследуемый от `ChosenInlineResult` и [`Context`](context.md).

**Вызывается, когда пользователь отправил инлайн-результат своему другу.**

_Важно: у вас должен быть включён `/setinlinefeedback` у **@BotFather**_
...
| `update`   | `TelegramChosenInlineResult` | [Объект события][TelegramChosenInlineResult] |

[TelegramChosenInlineResult]: https://core.telegram.org/bots/api#choseninlineresult
