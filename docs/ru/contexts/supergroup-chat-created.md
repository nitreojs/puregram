...
**Вызывается, когда создается групповой чат.**

```ts
import { SupergroupChatCreatedContext } from 'puregram';
```
...
```ts
const context: SupergroupChatCreatedContext = new SupergroupChatCreatedContext(telegram, update);
```
