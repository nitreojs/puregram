...
| `onTimeout` | `'return' \| 'throw' \| (raw) => void` | `'return'` | what happens when `timeoutMilliseconds` elapses before dispatch settles. `'return'` answers 200 — telegram treats the update as delivered and releases its per-chat queue, so the next update for that chat may arrive while this one is still running. `'throw'` answers 500 so telegram redelivers, and routes a `WebhookTimeout` through `tg.catch`; pair it with `dedupeUpdates`, or the still-running dispatch and the redelivered one both process the same update |
