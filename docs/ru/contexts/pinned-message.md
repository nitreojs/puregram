...
**Вызывается, когда в диалоге закрепляется сообщение.**

```ts
import { PinnedMessageContext } from 'puregram';
```
...
**Возвращает сообщение, которое было закреплено.**

```ts
context.eventMessage // => MessageContext
```
